...
const Login = () => {
    ...
    const [Cookies, setcookie, removeCookie] = useCookies(['id_save']);
    ...
    const id_save = () => {
        const icon = document.querySelector(".Login_id_save_icon");
        if(icon.innerHTML === '') {
            icon.innerText = '✔';
        } else {
            icon.innerText = '';
    ...
    let onLogin = async function () {
        ...
        if(id.value != '' && password.value != '') {
            try {
                let res = await axios({
                    method: 'POST',
                    url: 'https://freshman.entrydsm.hs.kr/users/login',
                    data: {
                        account_id: id.value,
                        password: password.value,
                    },
                });
                setCookie('access_token',res.data['access_token'], { path: '/', expires:  new Date(Date.now() + 86400000) });
                if(icon.innerHTML == '✔'){
                    setcookie('id_save',id.value, { path: '/', expires:  new Date(Date.now() + 8640000000) });
                } else {
    ...
    return (
        <div className="Login_Box">
            ...
            <span className="Login_Small_Title">로그인</span>
            <input type='text' className="Login_id"
              placeholder="아이디를 입력해 주세요."
              onKeyUp={() => btn_clear()}  
            />
            <span className="Login_Small_Title">비밀번호</span>
            ...
            <div className="Login_id_save" onClick={() => id_save()}>
                <div className="Login_id_save_icon"></div>
                <span>아이디 저장</span>
            </div>
            <button className="Login_btn" onClick={() => onLogin()}>완료</button>
        </div>
    )
}
